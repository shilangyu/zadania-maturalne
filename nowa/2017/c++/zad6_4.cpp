...
int main() {
    ...
    if (file.is_open()) {
        while (getline(file, element, ' ')) {
            ...
            if (width < 320 and height < 200) board[height][width] = element;
	...
	for (int i = 0; i < 320; i++) {
		...
		for (int j = 0; j < verticalLines[i].size(); j++) {
			if (j == 0) currentLength++;
			else if (current[j] == current[j - 1]) currentLength++;
			else break;
		}
	...
	cout << "6.4 Liczba elementow najdluzszej kolumny pikseli swiecacej z ta sama jasnoscia: " << maxLength << endl;
