...
int main() {
	...
	for (int h = 0; h < 200; h++) {
		for (int w = 0; w < 320; w++) {
			...
			if (w != 0) {
				if (isContrasting(primeValue, atoi(board[h][w - 1].c_str()))) checker++;
			}
			...
			if (h != 0) {
				if (isContrasting(primeValue, atoi(board[h - 1][w].c_str()))) checker++;
			}
			...
			if (checker > 0) howManyContrastingPixels++; 
			checker = 0;
		}
	}

	cout << "6.3 Liczba kontrastujacych pikseli: " << howManyContrastingPixels << endl;
